...
public class GlassActivity extends AppCompatActivity implements DatePickerDialog.OnDateSetListener {
    ...
    @BindView(R.id.spnGlassType)
    Spinner spnGlassType;

    @BindView(R.id.spnGlassShape)
    Spinner spnGlassShape;
    ...
    @BindView(R.id.etserial_number)
    EditText etserial_number;
    ...
    public void Submit(){
       ...
       // spnGlassType model.(spnGlassType.getSelectedItemPosition());
      // spnGlassShape  model.(spnGlassShape.getSelectedItemPosition());
        model.setBrandTypeId(spnbrand.getSelectedItemPosition());
        model.setColorId(spnColor.getSelectedItemPosition());

        model.setEmcProductId(etSize.getText().toString());
        model.setModel(etModel.getText().toString());
        model.setSerialNo(etserial_number.getText().toString());
       // etAmmount model.(etAmmount.getText().toString());
            ...
            public void onResponse(Call<String> call, Response<String> response) {
                try {
                    if (response.body() != null) {
                        Toast.makeText(GlassActivity.this, "Successfully Done!", Toast.LENGTH_SHORT).show();
                        CodeGenerateActivity.code = response.body().toString();
                        Intent intent = new Intent(GlassActivity.this, CodeGenerateActivity.class);
                        startActivity(intent);
                        finish();
                    } else {
                        Toast.makeText(GlassActivity.this, "Invalid Credentials!", Toast.LENGTH_SHORT).show();
                    }
                } catch (Exception e) {
                    Toast.makeText(GlassActivity.this, "Something went Wrong! Please try again later", Toast.LENGTH_SHORT).show();
                }
